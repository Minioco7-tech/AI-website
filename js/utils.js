...
export function getUniqueCategories(models) {
  ...
  models.forEach(model => {
    const cats = Array.isArray(model.category) ? model.category : [model.category];
    cats.forEach(cat => categories.add(cat.toLowerCase()));
  });

  return [...categories];
}

export function filterBySelectedCategories(models, selectedCategories) {
  if (selectedCategories.size === 0) return models;

  return models.filter(model => {
    const modelCats = Array.isArray(model.category)
      ? model.category.map(c => c.toLowerCase())
      : [model.category.toLowerCase()];
    return [...selectedCategories].some(cat => modelCats.includes(cat));
  });
}

export function renderCategoryFilters(container, models, selectedCategories, onChange) {
  if (!container) return;

  container.innerHTML = '';
  const categories = getUniqueCategories(models).sort();
  ...
  categories.forEach(cat => {
    ...
    checkbox.addEventListener('change', () => {
      ...
      onChange(); // callback to refresh filtered results
    });

    label.appendChild(checkbox);
    label.append(` ${cat.charAt(0).toUpperCase() + cat.slice(1)}`);
